...
uint32_t DHT22_GetReadings(void) {
	...
	wait = 0;
	while ((DHT22_GPIO_PORT->IDR & DHT22_GPIO_PIN) != (uint32_t)Bit_RESET && (wait++ < 250)) Delay_us(1);
	...
	if ((wait < 12) || (wait > 20)) return DHT22_RCV_BAD_ACK1;

	wait = 0;
	while (((DHT22_GPIO_PORT->IDR & DHT22_GPIO_PIN) != (uint32_t)Bit_RESET) && (wait++ < 100)) Delay_us(1);
	if ((wait < 12) || (wait > 20)) return DHT22_RCV_BAD_ACK2;
