...
//		UART_SendHex8(response); UART_SendChar('\n');
	} while ((response & 0x80) && --wait);

    // FIXME: in case of timeout try to send dummy byte instead of always sending dummy
	if (wait == 0) {
		UART_SendStr("CMD timeout\n");
//		SPIx_SendRecv(SDCARD_SPI_PORT,0xff);
	}
...
SDResult_TypeDef SD_Cmd(uint8_t cmd, uint32_t arg, SDCmdResp_TypeDef resp_type, uint8_t *resp) {
	...
	// Read the response
	while (rdLen--) {
		*resp++ = SPIx_SendRecv(SDCARD_SPI_PORT,0xff);
	}
...
SDResult_TypeDef SD_Init(void) {
	...
	wait = 100;
	do {
		response = SD_Cmd(SD_CMD_GO_IDLE_STATE,0x00);
		UART_SendStr("IDLE: ");
		UART_SendHex8(response);
		UART_SendChar('\n');
	} while (!(response & SD_R1_IDLE) && --wait);
	if (!wait) {
		SDCARD_CS_H();
		return SDR_NoResponse;
	}

	while (SD_Cmd(SD_CMD_GO_IDLE_STATE,0x00) != SD_R1_IDLE && --wait);
	if (!wait) {
		SDCARD_CS_H();
		return SDR_NoResponse;
	}
	...
	if (response == 0x05) {
		...
		do {
			...
			if (response & SD_R1_ILLEGAL_CMD) break; // MMC will respond with R1 'illegal command'
			response = SD_Cmd(SD_CMD_SD_APP_OP_COND,0); // ACMD41
		} while (response != 0 && --wait);

		if (!wait || response & SD_R1_ILLEGAL_CMD) {
			// MMC or bad card
...
//	SD_ReadCID();
	...
	UART_SendStr("CSD: ");
	UART_SendHex8(SD_ReadCSD());
	UART_SendChar('\n');
	UART_SendStr("CID: ");
	UART_SendHex8(SD_ReadCID());
	UART_SendChar('\n');

	UART_SendStr("CardBlockSize = ");
...
//			(SDCard.CardType == SDCT_MMC)) && (SDCard.CardBlockSize != 512)) {
			(SDCard.CardType == SDCT_MMC))) {
		SDCARD_CS_L();
		// CMD16: block size = 512 bytes
		response = SD_Cmd(SD_CMD_SET_BLOCKLEN,512);
		SDCARD_CS_H();
		if (response != 0x00) return SDR_SetBlockSizeFailed;
	}
...
SDResult_TypeDef SD_ReadBlock(uint32_t addr, uint8_t *pBuf, uint32_t len) {
	...
	if (response == 0x00) {
		...
		// Card rejected CMD17
		SDCARD_CS_H();
		if (response & SD_R1_ADDR_ERROR) {
			// Incorrect address specified
			return SDR_AddrError;
		} else {
			// READ_SINGLE_BLOCK failed
			return SDR_ReadError;
		}
	}
...
SDResult_TypeDef SD_WriteBlock(uint32_t addr, uint8_t *pBuf, uint32_t len) {
	...
	// SD card accepts byte address while SDHC/SDXC accepts block address in multiples of 512
	// therefore block address must be converted into corresponding byte address
	if (SDCard.CardType != SDCT_SDHC) addr <<= 9;
//	response = SD_Cmd(SD_CMD_WRITE_SINGLE_BLOCK,addr); // CMD24
	response = SD_Cmd(SD_CMD_WRITE_SINGLE_BLOCK,0xfffffffd); // CMD24

	UART_SendStr("CMD24 R:");
	UART_SendHex8(response);
	UART_SendChar('\n');
	...
	if (response == 0x00) {
		...
		// Card rejected CMD24
		if (response & SD_R1_ADDR_ERROR) {
			// Incorrect address specified, issue stop command (CMD12)

			SDCARD_CS_L();
			UART_SendStr("CMD12 R1b:");
			response = SD_Cmd(SD_CMD_STOP_TRANSMISSION,0);
			UART_SendHex8(response);
			UART_SendChar('\n');
			wait = 0x7fff; // Recommended timeout is 250ms (500ms for SDXC) FIXME: 0x7fff is set by sight, need calculate more adequate value
			do {
