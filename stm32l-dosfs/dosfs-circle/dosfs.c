...
void* DFS_memset( void* dest, uint8_t c, int32_t count )
{
    uint32_t i;
...
void* DFS_memcpy( void* dest, void* src, int32_t count )
{
...
int32_t DFS_memcmp( const void* s1, const void* s2, uint32_t n )
{
    uint8_t u1, u2;
...
/*int32_t DFS_strlen (const uint8_t *str)
{
    int32_t n=0;
    const uint8_t *s = str;
...
void DFS_strcpy( uint8_t* Dest ,  uint8_t* Src )
{
    /*  uint8_t * d= Dest;*/
...
uint8_t* DFS_strncpy( uint8_t* s1, const uint8_t* s2, int32_t n )
{
    uint8_t c;
    uint8_t* s = s1;
...
int32_t DFS_strcmp( uint8_t* s1,  uint8_t* s2 )
{
    while ( *s1 == *s2 && *s1 != '\0' )
    {
        s1++;
...
uint32_t DFS_OpenDir( PVOLINFO volinfo, uint8_t* dirname, PDIRINFO dirinfo )
{
    ...
    /*  if (!DFS_strlen((uint8_t *) dirname) || (DFS_strlen((uint8_t *) dirname) == 1 && dirname[0] == DIR_SEPARATOR))   // YRT20080204*/
    if ( !strlen(( uint8_t* ) dirname ) || ( strlen(( uint8_t* ) dirname ) == 1 && dirname[0] == DIR_SEPARATOR ) )
    {
...
/*
    ...
    A deleted file will be returned as a NULL entry (first uint8_t of filename=0)
...
uint32_t DFS_OpenFile( PVOLINFO volinfo, uint8_t* path, uint8_t mode, uint8_t* scratch, PFILEINFO fileinfo )
{
    ...
    DFS_strncpy(( uint8_t* ) tmppath, ( uint8_t* ) path, MAX_PATH );
    tmppath[MAX_PATH - 1] = 0;
    if ( DFS_strcmp(( uint8_t* ) path, ( uint8_t* ) tmppath ) )
    {
        return DFS_PATHLEN;
    }
    ...
    while ( tmppath[0] == DIR_SEPARATOR )
        DFS_strcpy(( uint8_t* ) tmppath, ( uint8_t* ) tmppath + 1 );
...
uint32_t DFS_OpenFile(PVOLINFO volinfo, uint8_t *path, uint8_t mode, uint8_t *scratch, PFILEINFO fileinfo)
{
	...
	if (mode & DFS_WRITE) {
		...
		UART_SendStr("\nDFS_OpenFile(): WE KNOW THE FILE DOES NOT EXIST!\n");
		UART_SendStr("Locate or create a directory entry for this file\n");
		UART_SendStr("DE: ");
		UART_SendStr((char *)filename);
		UART_SendChar('\n');
		...
		de.wrtdate_l = 0x11;
		de.wrtdate_h = 0x34;
		de.attr = ATTR_ARCHIVE; // set ARCHIVE attribute to file

		// allocate a starting cluster for the directory entry
		cluster = DFS_GetFreeFAT(volinfo, scratch);
		UART_SendStr("Cluster:");
		UART_SendInt(cluster);
		UART_SendChar('\n');
